...
| 2026-06-16 | Removed all em-dashes from slide text throughout gen_ai_can_should.js (replaced with comma, colon, or split sentence per brand rule). Replaced ▶ Unicode arrows on slide 4 with react icon images (icArrow). Fixed slide 8 "hardens into" label: was unreadable at fontSize:6.5 in w:0.26" — replaced with white pill box (w:1.00", fontSize:9) centered over the gap. Updated SKILL.md: em-dash rule now explicitly covers source ingestion; added emoji/Unicode prohibition rule; corrected arrow-gap guidance to never use Unicode symbols. |
| 2026-06-16 | Fixed slide 7 accent bar overlapping 2-line title: "From Text Message to Production App" (36 chars) wraps at 30pt in w:7.5" — added lines=2 to titleBar(), shifted cards sy:1.28→1.72, trimmed sh:3.5→3.10 so cards end at 4.82 above callout at y:4.88. |
