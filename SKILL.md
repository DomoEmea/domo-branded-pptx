...
- **Don't let multi-row content overflow the footer** — for N rows with height `rh` and gap `g`, calculate the last row's bottom: `y0 + (N-1)*(rh+g) + rh`. This must be ≤ 5.0. Always compute this before writing the loop. Example: 5 rows at `rh:0.62, g:0.10, y0:1.38` → last row ends at `1.38 + 4×0.72 + 0.62 = 4.88` (safe). At `rh:0.68, g:0.16` the same 5 rows end at `5.42` — 1.42" overflow.
