...
- **Don't draw overlapping elements between column blocks — draw them AFTER all columns (z-order rule).** PptxGenJS renders elements in add order: later = on top. Any element that visually sits on top of multiple shapes (an arrow between columns, a bridge label, a pill that spans a column boundary) must be added to the slide **after** all the shapes it overlaps. If it is added before even one of those shapes, that shape will be drawn on top and partially or fully cover the element. Pattern: collect all column/panel code first, then add all overlay elements (arrows, labels, pills) at the end.
- **Don't use a gray separator line in the footer — it must always be Domo Blue, starting after the badge.** `addFooter()` uses `x:0.80, y:5.15, w:8.70, h:0, line:{color:C.domoBlue, width:0.5}`. Starting at `x:0.80` places the line just after the DOMO icon badge (badge ends at `0.35+0.38=0.73"`), keeping the line visually clear of the icon. The line ends at `x:9.50`. Never start the line at `x:0.50` — that cuts under the badge. Never substitute `C.neutral3` or any other color for the footer separator.
- **Don't leave icon circles top-biased in short side-by-side cards.** In `card()` with `fullWidthBody:false` (the default side-by-side icon/title layout), the circle is vertically centered in the card height: `cy = y + (h - cs) / 2`. For a `h:1.10` card with `cs:0.46`, this gives `cy = y + 0.32` (circle centered in the tile). A fixed `cy = y + 0.18` only looks correct on tall cards; on short cards (`h ≤ 1.5"`) the circle drifts to the top with empty space below. The `fullWidthBody:true` variant keeps `cy = y + 0.18` (icon is in the header zone above body text) — do not change that case.
