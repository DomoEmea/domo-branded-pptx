...
- **Don't use a gray separator line in the footer — it must always be Domo Blue.** `addFooter()` uses `line:{color:C.domoBlue, width:0.5}` for the thin separator at `y:5.15`. The width is always `x:0.5, w:9.0` (full content width). Never substitute `C.neutral3` or any other color for the footer separator line.
